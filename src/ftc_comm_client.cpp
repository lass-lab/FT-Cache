...
/* RPC Globals */
static hg_id_t ftc_client_rpc_id;
static hg_id_t ftc_client_open_id;
static hg_id_t ftc_client_close_id;
static hg_id_t ftc_client_seek_id;
ssize_t read_ret = -1;
...
static hg_return_t
ftc_seek_cb(const struct hg_cb_info *info)
{
    ftc_seek_out_t out;
...
static hg_return_t
ftc_open_cb(const struct hg_cb_info *info)
{
    ftc_open_out_t out;
	...
	strncpy(log_info.filepath, ftc_open_state_p->filepath, sizeof(log_info.filepath));
    strncpy(log_info.request, "open", sizeof(log_info.request));
    log_info.client_rank = client_rank;
    log_info.server_rank = ftc_open_state_p->svr_hash;
    strncpy(log_info.expn, "CReceive", sizeof(log_info.expn));
    log_info.n_epoch = ftc_open_state_p->local_fd;
    log_info.n_batch = -1;
    ...
    /* Signal to main() that we are done */
    pthread_mutex_lock(ftc_open_state_p->mutex);
    *(ftc_open_state_p->done) = HG_TRUE;
    pthread_cond_signal(ftc_open_state_p->cond);
    pthread_mutex_unlock(ftc_open_state_p->mutex);

    free(ftc_open_state_p);
...
static hg_return_t
ftc_read_cb(const struct hg_cb_info *info)
{
	hg_return_t ret;
    ftc_rpc_out_t out;
	...
	snprintf(log_info.filepath, sizeof(log_info.filepath), "fd_%d", ftc_rpc_state_p->local_fd);
    strncpy(log_info.request, "read", sizeof(log_info.request));
    log_info.client_rank = client_rank;
    log_info.server_rank = ftc_rpc_state_p->svr_hash;
    strncpy(log_info.expn, "CReceive", sizeof(log_info.expn));
    log_info.n_epoch = -1;
    log_info.n_batch = -1;
    gettimeofday(&log_info.clocktime, NULL);
   ...
   /* Clean up resources consumed by this rpc */
    ...
    /* Signal to main() that we are done */
	pthread_mutex_lock(ftc_rpc_state_p->mutex);
    *(ftc_rpc_state_p->done) = HG_TRUE;
    pthread_cond_signal(ftc_rpc_state_p->cond);
    pthread_mutex_unlock(ftc_rpc_state_p->mutex);	
...
{   
    ftc_client_open_id = ftc_open_rpc_register();
    ftc_client_rpc_id = ftc_rpc_register();    
    ftc_client_close_id = ftc_close_rpc_register();
    ftc_client_seek_id = ftc_seek_rpc_register();
}
...
void ftc_client_block(uint32_t host, hg_bool_t *done, pthread_cond_t *cond, pthread_mutex_t *mutex)
{
    ...
    while (*done != HG_TRUE){
	    ...
	    if (wait_status == ETIMEDOUT){
		    L4C_INFO("TIMEOUT: Timeout period elapsed in ftc_client_block");
...
ssize_t ftc_read_block(uint32_t host, hg_bool_t *done, ssize_t *bytes_read, pthread_cond_t *cond, pthread_mutex_t *mutex)
{
    ...
    while (*done != HG_TRUE) {
		wait_status = pthread_cond_timedwait(cond, mutex, &timeout);
        if (wait_status == ETIMEDOUT){
	    	L4C_INFO("TIMEOUT: FT_Cache remote read failed due to timeout; it will be redirected to pfs");
			/* Timeout counter update */
			{
				std::lock_guard<std::mutex> lock(timeout_mutex);
            	timeout_counters[host]++;
    ...
    pthread_mutex_unlock(mutex);
	if (result < 0) {
        L4C_INFO("FT_Cache remote read failed with error %zd; %zd returned\n", result, result);
        return result;
    }
    return result;
...
{   
    ...
    ret = HG_Addr_to_string(ftc_comm_get_class(), server_addr, &server_addr_str_size, svr_addr);
    char server_ip[128];
    extract_ip_portion(server_addr, server_ip, sizeof(server_ip));
...
void ftc_client_comm_gen_open_rpc(uint32_t svr_hash, string path, int fd, ftc_open_state_t *ftc_open_state_p)
{
    ...
    ftc_open_state_p->local_fd = fd;

    /* Create handle to represent this rpc operation */    
    ftc_comm_create_handle(svr_addr, ftc_client_open_id, &handle);  
...
void ftc_client_comm_gen_read_rpc(uint32_t svr_hash, int localfd, void *buffer, ssize_t count, off_t offset, ftc_rpc_state_t_client *ftc_rpc_state_p)
{
	...
	ftc_rpc_state_p->bulk_handle = HG_BULK_NULL;

    /* Create handle to represent this rpc operation */
    ftc_comm_create_handle(svr_addr, ftc_client_rpc_id, &(ftc_rpc_state_p->handle));

    /* Register buffer for rdma/bulk access by server */
    hgi = HG_Get_info(ftc_rpc_state_p->handle);
    assert(hgi);
    ret = HG_Bulk_create(hgi->hg_class, 1, (void**) &(buffer),
       &(ftc_rpc_state_p->size), HG_BULK_WRITE_ONLY, &(in.bulk_handle));
...
void ftc_client_comm_gen_seek_rpc(uint32_t svr_hash, int fd, int offset, int whence)
{
    ...
    ret = HG_Forward(handle, ftc_seek_cb, NULL, &in);
    assert(ret == 0);
...
hg_addr_t ftc_client_comm_lookup_addr(int rank)
{
	if (address_cache.find(rank) != address_cache.end())
	{
        ...
        HG_Addr_lookup2(ftc_comm_get_class(), address_cache[rank].c_str(), &target_server);
...
//	char *stepid = getenv("PMI_NAMESPACE");
	...
	if (svr_found){
        ...
        HG_Addr_lookup2(ftc_comm_get_class(),svr_str,&target_server);		
	}
...
void ftc_get_addr() {
    ...
    if(my_address == HG_ADDR_NULL){
        ...
        HG_Addr_self(ftc_comm_get_class(), &client_addr);
        HG_Addr_to_string(ftc_comm_get_class(), addr_buffer, &size, client_addr);
        HG_Addr_free(ftc_comm_get_class(), client_addr);

        std::string address = std::string(addr_buffer);
        HG_Addr_lookup2(ftc_comm_get_class(), address.c_str(), &my_address);
