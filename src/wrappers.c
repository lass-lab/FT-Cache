 ...
 *
 * Updated:    01/08/2021 - Purpose moved to FT_Cache modified to build without 
 *             warning under C++ : Skeleton for FT_Cache Built and configured
 * 
 ...
 *
 * This file is part of the FT_Cache project.
 ****************************************************************************/
...
// segfault. (ie: fopen() calls ftc_init()) which needs to write a log
...
FILE *WRAP_DECL(fopen)(const char *path, const char *mode)
{
	...
	if (ptr != NULL)
	{
		if (ftc_track_file(path, O_RDONLY, fileno(ptr)))
		{
...
int WRAP_DECL(open)(const char *pathname, int flags, ...)
{
	...
	if (ret != -1){
		if (ftc_track_file(pathname, flags, ret))
		{
...
int WRAP_DECL(close)(int fd)
{
	...
	/* Check if ftc data has been initialized? Can we possibly hit a close call before an open call? */
	MAP_OR_FAIL(close);
...
ssize_t WRAP_DECL(read)(int fd, void *buf, size_t count)
{
	...
	ret = ftc_remote_read(fd,buf,count);
...
off_t WRAP_DECL(lseek)(int fd, off_t offset, int whence)
{
	...
	if (ftc_file_tracked(fd)){
		...
		return ftc_remote_lseek(fd,offset,whence);
	}
