 ...
 * 
 */
#ifndef __FT_Cache_INTERNAL_H__
#define __FT_Cache_INTERNAL_H__
...
#define FT_Cache_VERSION "0.0.1"

#ifdef FT_Cache_PRELOAD
...
{ \
    ...
    if(!(__real_ ## func)) { \
        fprintf(stderr, "ftc failed to map symbol: %s\n", #func); \
        exit(1); \
    } \
}
...
#endif
/* FT_Cache Internal API */
#ifdef __cplusplus
extern "C" bool ftc_track_file(const char* path, int flags, int fd);
extern "C" const char * ftc_get_path(int fd);
extern "C" bool  ftc_remove_fd(int fd);
extern "C" ssize_t ftc_remote_read(int fd, void *buf, size_t count);
extern "C" ssize_t ftc_remote_pread(int fd, void *buf, size_t count, off_t offset);
extern "C" ssize_t ftc_remote_lseek(int fd, int offset, int whence);
extern "C" void ftc_remote_close(int fd);
extern "C" bool ftc_file_tracked(int fd);
#endif

extern bool ftc_track_file(const char* path, int flags, int fd);
extern const char * ftc_get_path(int fd);
extern bool  ftc_remove_fd(int fd);
extern ssize_t ftc_remote_read(int fd, void *buf, size_t count);
extern ssize_t ftc_remote_pread(int fd, void *buf, size_t count, off_t offset);
extern ssize_t ftc_remote_lseek(int fd, int offset, int whence);
extern void ftc_remote_close(int fd);
extern bool ftc_file_tracked(int fd);
