...
void ftc_init_logging()
{
    ...
    if (getenv(FT_Cache_LOG_LEVEL))
    {
        int level = atoi(getenv(FT_Cache_LOG_LEVEL));
    ...
    char *param_log_dir = getenv(FT_Cache_LOG_DIR);
    if (param_log_dir == NULL) {
        param_log_dir = L4C_LOG_DIR;
    }
    char *param_log_prefix = getenv(FT_Cache_LOG_PREFIX);
...
void FT_Cache_finalize_logging()
{
    log4c_fini();
}
